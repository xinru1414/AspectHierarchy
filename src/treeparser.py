...
def get_noun_chunks(doc: spacy.tokens.doc.Doc) -> List:
    ...
    for chunk in doc.noun_chunks:
        if len(chunk) > 1:
            ...
            # filtering out pronouns
            if str(chunk).lower() not in ['i', 'mine', 'me', 'you', 'yours', 'they', 'them', 'theirs', 'he', 'him', 'his', 'she', 'her', 'hers', 'it', 'its', 'we', 'us', 'ours']:
                chunks.append(str(chunk))
    return chunks
...
def relation_based_pairs_with_meta(d: Dict, primary_aspects: List, key_word: str) -> Dict:
    '''
        ...
        for j, item in enumerate(values['noun_pairs']):
            ...
            for value in item:
                if ((value[0] in primary_aspects) or (len([i for i in value[0].lower().split() if i in primary_aspects]) == 1)) and (
                        key_word not in value[1].lower().split()) and len(list(set(value[0].lower().split()) & set(value[1].lower().split()))) == 0:
...
def print_brand_pairs_only(d: Dict, not_cared_aspects: List, determiners: List):
    '''
    ...
    for key, item in d.items():
        ...
        for x, y in item['relation_based_pairs']:
            x = ' '.join([i for i in x.split() if (i not in determiners) and (i not in ignore)])
            x = ''.join([i for i in x if i not in string.punctuation]).strip()
            y = ' '.join([i for i in y.split() if (i not in determiners) and (i not in ignore)])
            y = ''.join([i for i in y if i not in string.punctuation]).strip()
...
def get_brand_pairs(d: Dict, primary_aspects: List, not_cared_aspects: List, determiners: List) -> Set:
    '''

    :param d: Creates consise aspect pairs for aspect hierarchy generation from dictionary generated by relation_based_pairs_with_meta
    :param primary_aspects: a list of cared aspects (see data/resources/primary_aspects)
    :param not_cared_aspects: a list of ignored aspects (see data/resources/not_cared_aspects)
    :param determiners: a list of determiners to filter out (see data/resources/determiners)
    :return: a set of secondary aspect pairs
    '''
    pairs = []
    ignore = not_cared_aspects + [i for i in string.punctuation]
    for key, item in d.items():
